...
["keyword"])
 ...
 for rows.Next() {
 if err := rows.Scan(&mahasiswa.Id, &mahasiswa.Npm, &mahasiswa.Nama, &maha
siswa.Kelas, &mahasiswa.Profile); err != nil {
 log.Print(err.Error())
 }
 mhs = append(mhs, mahasiswa)
 }
...
func createMahasiswa(w http.ResponseWriter, r *http.Request) {
 ...
 profile := "gambar1.jpg"
 rows, err := db.Prepare("INSERT INTO mahasiswa(npm, nama, kelas, profile) VAL
UES(?, ?, ?, ?)")
 if err != nil {
 log.Print(err.Error())
 }
...
func updateMahasiswa(w http.ResponseWriter, r *http.Request) {
 ...
 update, err := db.Prepare("UPDATE mahasiswa SET npm=?, nama=?, kelas=? WHERE
id=?")
 if err != nil {
 log.Print(err.Error())
 }
